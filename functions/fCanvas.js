...
function reflectImage2Canvas(ctx, imageDom, previewAreaDomWidth, previewAreaDomHeight, dx, dy) {
  const ratio = getOrientation(imageDom.naturalWidth, imageDom.naturalHeight) === `landscape` ? previewAreaDomWidth / imageDom.naturalWidth : previewAreaDomHeight / imageDom.naturalHeight;
  ...
  ctx.drawImage(imageDom, dx + deltaParallelMoveX, dy + deltaParallelMoveY, imageDom.naturalWidth * ratio, imageDom.naturalHeight * ratio);

  return ctx;
}
exports.reflectImage2Canvas = reflectImage2Canvas;
...
async function teamStats(clientMongo, league, mongoTeam, mongoRanking) {
  ...
  let myCanvas = Canvas.createCanvas(config.canvasSize.width, config.canvasSize.height);
  ...
  // ***** background image ***** //
  bgUrl = `./image/bg/war_${league}.png`;
  ...
  const myFont = 'Optima';
  Canvas.GlobalFonts.registerFromPath('./fonts/Noto_Serif_JP/NotoSerifJP-Black.otf', 'NotoSerifJP');
  Canvas.GlobalFonts.registerFromPath('./fonts/SourceHanSans-VF.ttf', 'HanSans');
  const myFontJP = 'NotoSerifJP';
  Canvas.GlobalFonts.registerFromPath('./fonts/Supercell-magic.ttf', 'Supercell');
  const myFontSC = 'Supercell';
  Canvas.GlobalFonts.registerFromPath('./fonts/Graduate-Regular.ttf', 'Graduate');
  Canvas.GlobalFonts.registerFromPath('./fonts/Kameron-VariableFont_wght.ttf', 'Kameron');
  const myFontNumber = 'Kameron';
  ...
  ctx.font = `${config.canvasFontSize.small}px ${myFont}`;
  ctx.fillText(text, widthCenter, 200);

  // team logo
  const lengthLogo = 400;
  const posLogo = { x: widthCenter - lengthLogo / 2, y: pos.v151 - lengthLogo / 2 };
  //const lengthLogo2 = 3500;
  //const posLogo2 = { x: pos.h152 - lengthLogo2 / 2, y: pos.v162 - lengthLogo2 / 2 };
  try {
    const imgTeam = await Canvas.loadImage(`./image/teamLogo/${mongoTeam.clan_abbr}.png`);
    //ctx.globalAlpha = 0.2;
    //ctx = reflectImage2Canvas(ctx, imgTeam, lengthLogo2, lengthLogo2, posLogo2.x, posLogo2.y);
    ctx.globalAlpha = 1.0;
    ctx = reflectImage2Canvas(ctx, imgTeam, lengthLogo, lengthLogo, posLogo.x, posLogo.y);
  }
  catch (error) {
    const imgJwc = await Canvas.loadImage(`./image/JWC.png`);
    ctx = reflectImage2Canvas(ctx, imgJwc, lengthLogo, lengthLogo, posLogo.x, posLogo.y);
  };

  // team name
  ctx.fillStyle = config.rgb.gray;
  ...
  if (ratz.test(text.charAt(0)) || rAtZ.test(text.charAt(0)) || r0t9.test(text.charAt(0))) {
    ;
  }
  else {
    if (text.length > 14) {
  ...
  if (mongoTeam.clan_abbr.includes('omi') || mongoTeam.clan_abbr.includes('lily')) {
    ctx.font = `${fontSize_teamName}px ${myFontJP}`;
  }
  else {
    ctx.font = `${fontSize_teamName}px ${myFont}, ${myFontJP}, sans-serif`;
  };
  ctx.fillText(text, pos.h111, pos.v151);

  // league
  text = config.league[league];
  ctx.font = `${config.canvasFontSize.large}px ${myFont}`;
  ...
  if (mongoTeam.score.result == 1) {
    ...
    ctx.drawImage(imgTrophy, pos.h102 - lengthTrophy / 2, pos.v171 - lengthTrophy / 2, lengthTrophy, lengthTrophy);
  };

  // subtitles
  ctx.textAlign = 'center';
  ctx.font = `${config.canvasFontSize.medium}px ${myFont}`;
  //text = 'STATS';
  ...
  ctx.font = `${config.canvasFontSize.xLarge}px ${myFont}`;
  ...
  ctx.drawImage(imgStar, pos.h61 - lengthStar / 2, pos.v111 - lengthStar / 2 - 20, lengthStar, lengthStar);
  ...
  if (league == 'j1' || league == 'j2') {
    ...
    ctx.drawImage(imgStars, posImgStarsH, posImgStarsV, 300 * imgRatio, 130 * imgRatio);
    var imgRatio = 0.35;
    var imgStars = await Canvas.loadImage(`./image/scStarTripleBlue.png`);
    var posImgStarsH = pos.h121 + 20 - 300 * imgRatio / 2;
    ctx.drawImage(imgStars, posImgStarsH, posImgStarsV + 10, 300 * imgRatio, 130 * imgRatio);
    ...
    ctx.font = `${config.canvasFontSize.xSmall}px ${myFont}`;
    ...
    ctx.textBaseline = 'middle';
    ctx.font = `${config.canvasFontSize.medium}px ${myFont}`;
    ctx.fillText(`${Math.round(mongoTeam.score.sum.clan.allAttackTypes.hitrate.total)}`, pos.h101 + 10, posHirtateV);
    ctx.textAlign = 'left';
    ctx.font = `${config.canvasFontSize.small}px ${myFont}`;
    ...
    ctx.textAlign = 'right';
    ctx.font = `${config.canvasFontSize.medium}px ${myFont}`;
    ctx.fillText(`${mongoTeam.score.sum.clan.allAttackTypes.nTriple.total}`, pos.h101 + 0, pos_triple - 10);
    ctx.textAlign = 'left';
    ctx.font = `${config.canvasFontSize.small}px ${myFont}`;
    ctx.fillText(`/${mongoTeam.score.sum.clan.allAttackTypes.nAt.total}`, pos.h101 + 0, pos_triple);
  }
  else if (league == 'mix') {
    const imgRatio = 0.5;
    ...
    ctx.textAlign = 'right';
    ctx.font = `${config.canvasFontSize.medium}px ${myFont}`;
    ctx.fillText(`${Math.round(mongoTeam.score.sum.clan.allAttackTypes.hitrate.total)}`, pos.h161 + 30, posHirtateV);
    ctx.fillText(`${mongoTeam.score.sum.clan.allAttackTypes.nTriple.total}`, pos.h161 + 20, posTripleV - 10);
    ctx.textAlign = 'left';
    ctx.font = `${config.canvasFontSize.small}px ${myFont}`;
    ctx.fillText('%', pos.h161 + 30, posHirtateV + 5);
    ctx.fillText(`/${mongoTeam.score.sum.clan.allAttackTypes.nAt.total}`, pos.h161 + 20, posTripleV);

    // タウンホール別
    const imgLength = 80;
    const townhalls = [
      { level: 'th17', img: './image/th17_cam3.png', posH: pos.h121, posV: pos.v101 },
      { level: 'th16', img: './image/th16_cam3.png', posH: pos.h101, posV: pos.v101 },
      { level: 'th15', img: './image/th15_cam3.png', posH: pos.h81, posV: pos.v101 + 5 },
      { level: 'th14', img: './image/th14_cam3.png', posH: pos.h61, posV: pos.v101 },
      { level: 'th13', img: './image/th13_cam3.png', posH: pos.h41, posV: pos.v101 }
    ];
    ...
    for (const th of townhalls) {
      ...
      ctx.drawImage(imgTH, posImgTownhallH, posImgTownhallV, imgLength, imgLength);

      // ヒットレートを描画
      ctx.textAlign = 'right';
      ctx.font = `${config.canvasFontSize.small}px ${myFont}`;
      ctx.fillText(`${Math.round(mongoTeam.score.sum.clan.allAttackTypes.hitrate[th.level])}`, th.posH + 40, posHirtateV - 3);
      ctx.fillText(`${mongoTeam.score.sum.clan.allAttackTypes.nTriple[th.level]}`, th.posH + 20, posTripleV - 5);
      ctx.textAlign = 'left';
      ctx.font = `${config.canvasFontSize.xxSmall}px ${myFont}`;
      ctx.fillText('%', th.posH + 40, posHirtateV + 2);
      ctx.fillText(`/${mongoTeam.score.sum.clan.allAttackTypes.nAt[th.level]}`, th.posH + 20, posTripleV);
    };
  };
  ...
  let chartDataTeam = [];
  if (league == 'j1' || league == 'j2') {
    chartLabels = ['All att.', 'Fresh att.', 'Cleanup att.', 'Overkill att.', 'All def.', 'Fresh def.', 'Cleanup def.', 'Overkill def.'];
    ...
    chartLabels = ['TH17 att.', 'TH16 att.', 'TH15 att.', 'TH14 att.', 'TH13 att.', 'TH17 def.', 'TH16 def.', 'TH15 def.', 'TH14 def.', 'TH13 def.'];
  ...
  const chartColor = { j1: config.rgb.red, j2: config.rgb.blue, swiss: config.rgb.yellow, mix: config.rgb.green, five: config.rgb.purple.default };
  const chartColorAlpha = { j1: config.rgba.red, j2: config.rgba.blue, swiss: config.rgba.yellow, mix: config.rgba.green, five: config.rgb.purple.one };
  const chartData = {
    labels: chartLabels,
    datasets: [{
  ...
  ctx.drawImage(imgChart, pos.h102 - widthChart / 2, pos.v101 - heightChart / 2 + 20, widthChart, heightChart);
  // ***** Chart ***** //
  ...
  const query = { season: config.season[league], league: league, $or: [{ clan_abbr: mongoTeam.clan_abbr }, { opponent_abbr: mongoTeam.clan_abbr }] };
  const options = { projection: { _id: 0, clan_abbr: 1, opponent_abbr: 1, week: 1 } };
  const sort = {};
  const cursor = clientMongo.db('jwc').collection('wars').find(query, options).sort(sort);
  ...
  if (league == 'j1' || league == 'j2') {
    posStart_topPlayers = { total: pos.v41, fresh: pos.v42 + 48, cleanup: pos.v92, overkill: pos.v132 + 48 };
    ctx.textAlign = 'center';
    var imgRatio = 0.35;
    var imgStars = await Canvas.loadImage(`./image/scStarTriple.png`);
    ...
    ctx.drawImage(imgStars, posImgStarsH + 17, posImgStarsV, 300 * imgRatio, 130 * imgRatio);
    var imgStars = await Canvas.loadImage(`./image/scStarTripleOrange.png`);
    var posImgStarsV = posStart_topPlayers.cleanup - 130 * imgRatio / 1.4 + 2;
    ctx.drawImage(imgStars, posImgStarsH + 17, posImgStarsV, 300 * imgRatio, 130 * imgRatio);
    var imgStars = await Canvas.loadImage(`./image/scStarTripleRed.png`);
    var posImgStarsV = posStart_topPlayers.overkill - 130 * imgRatio / 1.4 + 2;
    ctx.drawImage(imgStars, posImgStarsH + 17, posImgStarsV, 300 * imgRatio, 130 * imgRatio);
    ...
    mongoRanking[league].total.forEach((acc, index) => {
      ...
      if (acc.homeClanAbbr == mongoTeam.clan_abbr) {
        ...
        ctx.font = `${config.canvasFontSize.xSmall}px ${myFont}, ${myFontJP}, sans-serif`;
    ...
    mongoRanking[league].fresh.forEach((acc, index) => {
      ...
      if (acc.homeClanAbbr == mongoTeam.clan_abbr) {
        ...
        if (acc.rank < 100) {
          text = `(${String(acc.rank)})`;
          ctx.font = `${config.canvasFontSize.xxxSmall}px ${myFont}`;
          ctx.fillText(text, pos.h42, pos_ranking);
        };
        text = String(acc.name);
        ctx.font = `${config.canvasFontSize.xxSmall}px ${myFont}, ${myFontJP}, sans-serif`;
        ctx.fillText(text, pos.h102, pos_ranking);
        ctx.textAlign = 'right';
        ctx.font = `${config.canvasFontSize.xSmall}px ${myFont}`;
        text = String(acc.nTriples);
  ...
  ctx.drawImage(imgJwcLogo, widthCenter - lengthLogoJwc / 2, heightCanvas - 260, lengthLogoJwc, lengthLogoJwc);
  ...
  const timeJst = utcToZonedTime(timeUtc, 'Asia/Tokyo');
  ctx.font = `italic ${config.canvasFontSize.xxxSmall}px ${myFont}`;
  text = format(timeJst, 'pp, PPPP') + ' [JST]';
  ctx.fillText(text, pos.h111, heightCanvas - 110);
  ctx.font = `italic ${config.canvasFontSize.xxxxSmall}px ${myFont}`;
  ...
  const attachment = new AttachmentBuilder(pngData, { name: `teamStats_${league}.png` });
...
async function standings(league, standings, leagueStats) {
  ...
  const imgJwcLogo = await Canvas.loadImage('./image/JWC.png');
  ctx.drawImage(imgJwcLogo, widthCenter - lengthLogoJwc / 2, heightCanvas - 200, lengthLogoJwc, lengthLogoJwc);
  ...
  if (league == 'j1' || league == 'j2') {
    ...
    */
  ...
  else if (league == 'mix') { // change TH level
    text = 'TH17';
    ...
    ctx = reflectImage2Canvas(ctx, imgTh17, lengthTh, lengthTh, posH.th17 - lengthTh / 2 + 30, headerHeight + lengthTh / 4);
    const imgTh16 = await Canvas.loadImage(`./image/th16_cam3.png`);
    ctx = reflectImage2Canvas(ctx, imgTh16, lengthTh, lengthTh, posH.th16 - lengthTh / 2 + 30, headerHeight + lengthTh / 4);
    const imgTh15 = await Canvas.loadImage(`./image/th15_cam3.png`);
    ctx = reflectImage2Canvas(ctx, imgTh15, lengthTh, lengthTh, posH.th15 - lengthTh / 2 + 30, headerHeight + lengthTh / 2.9);
    const imgTh14 = await Canvas.loadImage(`./image/th14_cam3.png`);
    ctx = reflectImage2Canvas(ctx, imgTh14, lengthTh, lengthTh, posH.th14 - lengthTh / 2 + 30, headerHeight + lengthTh / 3.2);
    const imgTh13 = await Canvas.loadImage(`./image/th13_cam3.png`);
    ctx = reflectImage2Canvas(ctx, imgTh13, lengthTh, lengthTh, posH.th13 - lengthTh / 2 + 30, headerHeight + lengthTh / 3.0);
  };
  ctx.fillStyle = config.rgb.gray;
  ctx.font = `${config.canvasFontSize.xxxxSmall}px ${myFont}`;
  ...
  ctx = reflectImage2Canvas(ctx, imgStar, lengthStar, lengthStar, posH.sd - lengthStar / 2 - 20, headerHeight - lengthStar / 2);
  ...
  await Promise.all(standings.map(async (team, index) => {
    ...
    try {
      const imgTeam = await Canvas.loadImage(`./image/teamLogo/${team.clan_abbr}.png`);
      ctx = reflectImage2Canvas(ctx, imgTeam, lengthLogo, lengthLogo, dxA, dyA);
    }
    catch (error) {
      const imgJwc = await Canvas.loadImage(`./image/JWC.png`);
      ctx = reflectImage2Canvas(ctx, imgJwc, lengthLogo, lengthLogo, dxA, dyA);
    };
    ...
    if (team.clan_abbr.includes('omi') || team.clan_abbr.includes('lily')) {
      ctx.font = `${fontSizeTeamName}px ${myFontJP}`;
    }
    else {
      ctx.font = `${fontSizeTeamName}px ${myFont}, ${myFontJP}, sans-serif`;
    };
    ...
    if (score) {
      ...
      ctx.font = `${config.canvasFontSize.smallMedium}px ${myFont}`;
      ctx.fillText(text, posH.score, pos0);

      text = score.starDifference < 0 ? `${score.starDifference}` : `+${score.starDifference}`;
      ctx.font = `${config.canvasFontSize.smallMedium}px ${myFont}`;
      ...
      if (league == 'j1' || league == 'j2') {
        ctxHitrate(ctx, score.clan.allAttackTypes, 'total', posH.hitrate[league], pos0, myFont, config.canvasFontSize.smallMedium, config.canvasFontSize.xSmall, hMargin = 15);
        ctxTriples(ctx, score.clan.allAttackTypes, 'total', posH.hitrate[league], pos0, myFont, config.canvasFontSize.small, config.canvasFontSize.xxSmall, color = config.rgb.gray, hMargin = 10, vMargin = spacing / 4);

        ctxHitrate(ctx, score.clan.fresh, 'total', posH.fresh, pos0, myFont, config.canvasFontSize.small, config.canvasFontSize.xxSmall, hMargin = 15);
        ctxTriples(ctx, score.clan.fresh, 'total', posH.fresh, pos0, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxSmall, color = config.rgb.gray, hMargin = 10, vMargin = spacing / 4);

        ctxHitrate(ctx, score.clan.cleanup, 'total', posH.cleanup, pos0, myFont, config.canvasFontSize.small, config.canvasFontSize.xxSmall, hMargin = 15);
        ctxTriples(ctx, score.clan.cleanup, 'total', posH.cleanup, pos0, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxSmall, color = config.rgb.gray, hMargin = 10, vMargin = spacing / 4);

        ctxHitrate(ctx, score.clan.overkill, 'total', posH.overkill, pos0, myFont, config.canvasFontSize.small, config.canvasFontSize.xxSmall, hMargin = 15);
        ctxTriples(ctx, score.clan.overkill, 'total', posH.overkill, pos0, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxSmall, color = config.rgb.gray, hMargin = 10, vMargin = spacing / 4);
      }
      else if (league == 'swiss' || league == 'five') {
        ctxHitrate(ctx, score.clan.allAttackTypes, 'total', posH.hitrate[league], pos0, myFont, config.canvasFontSize.smallMedium, config.canvasFontSize.xSmall, hMargin = 15);
        ctxTriples(ctx, score.clan.allAttackTypes, 'total', posH.triple, pos0, myFont, config.canvasFontSize.smallMedium, config.canvasFontSize.xSmall, color = config.rgb.snowWhite, hMargin = 0, vMargin = 0);
      }
      else if (league == 'mix') { // change TH level
        ctxHitrate(ctx, score.clan.allAttackTypes, 'total', posH.hitrate[league], pos0, myFont, config.canvasFontSize.smallMedium, config.canvasFontSize.xSmall, hMargin = 30);
        ctxTriples(ctx, score.clan.allAttackTypes, 'total', posH.hitrate[league], pos0, myFont, config.canvasFontSize.small, config.canvasFontSize.xxxSmall, color = config.rgb.gray, hMargin = 20, vMargin = spacing / 4);

        ctxHitrate(ctx, score.clan.fresh, 'th17', posH.th17, pos0, myFont, config.canvasFontSize.small, config.canvasFontSize.xxxSmall, hMargin = 60);
        ctxTriples(ctx, score.clan.fresh, 'th17', posH.th17, pos0, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxxSmall, color = config.rgb.gray, hMargin = 50, vMargin = spacing / 4);

        ctxHitrate(ctx, score.clan.fresh, 'th16', posH.th16, pos0, myFont, config.canvasFontSize.small, config.canvasFontSize.xxxSmall, hMargin = 60);
        ctxTriples(ctx, score.clan.fresh, 'th16', posH.th16, pos0, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxxSmall, color = config.rgb.gray, hMargin = 50, vMargin = spacing / 4);

        ctxHitrate(ctx, score.clan.fresh, 'th15', posH.th15, pos0, myFont, config.canvasFontSize.small, config.canvasFontSize.xxxSmall, hMargin = 60);
        ctxTriples(ctx, score.clan.fresh, 'th15', posH.th15, pos0, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxxSmall, color = config.rgb.gray, hMargin = 50, vMargin = spacing / 4);

        ctxHitrate(ctx, score.clan.fresh, 'th14', posH.th14, pos0, myFont, config.canvasFontSize.small, config.canvasFontSize.xxxSmall, hMargin = 60);
        ctxTriples(ctx, score.clan.fresh, 'th14', posH.th14, pos0, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxxSmall, color = config.rgb.gray, hMargin = 50, vMargin = spacing / 4);

        ctxHitrate(ctx, score.clan.fresh, 'th13', posH.th13, pos0, myFont, config.canvasFontSize.small, config.canvasFontSize.xxxSmall, hMargin = 60);
        ctxTriples(ctx, score.clan.fresh, 'th13', posH.th13, pos0, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxxSmall, color = config.rgb.gray, hMargin = 50, vMargin = spacing / 4);
      };
    };
  ...
  if (leagueStats.stats) {
    if (league == 'j1' || league == 'swiss' || league == 'mix' || league == 'five') {
    ...
    if (statsLeague) {
      if (league == 'j1' || league == 'j2') {
        ctxHitrate(ctx, statsLeague.allAttackTypes, 'total', posH.hitrate[league], posAll, myFont, config.canvasFontSize.smallMedium, config.canvasFontSize.xSmall, hMargin = 15);
        ctxTriples(ctx, statsLeague.allAttackTypes, 'total', posH.hitrate[league], posAll, myFont, config.canvasFontSize.small, config.canvasFontSize.xxSmall, color = config.rgb.gray, hMargin = 10, vMargin = spacing / 4);

        ctxHitrate(ctx, statsLeague.fresh, 'total', posH.fresh, posAll, myFont, config.canvasFontSize.small, config.canvasFontSize.xxSmall, hMargin = 15);
        ctxTriples(ctx, statsLeague.fresh, 'total', posH.fresh, posAll, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxSmall, color = config.rgb.gray, hMargin = 10, vMargin = spacing / 4);

        ctxHitrate(ctx, statsLeague.cleanup, 'total', posH.cleanup, posAll, myFont, config.canvasFontSize.small, config.canvasFontSize.xxSmall, hMargin = 15);
        ctxTriples(ctx, statsLeague.cleanup, 'total', posH.cleanup, posAll, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxSmall, color = config.rgb.gray, hMargin = 10, vMargin = spacing / 4);

        ctxHitrate(ctx, statsLeague.overkill, 'total', posH.overkill, posAll, myFont, config.canvasFontSize.small, config.canvasFontSize.xxSmall, hMargin = 15);
        ctxTriples(ctx, statsLeague.overkill, 'total', posH.overkill, posAll, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxSmall, color = config.rgb.gray, hMargin = 10, vMargin = spacing / 4);
      }
      else if (league == 'swiss' || league == 'five') {
        ctxHitrate(ctx, statsLeague.allAttackTypes, 'total', posH.hitrate[league], posAll, myFont, config.canvasFontSize.smallMedium, config.canvasFontSize.xSmall, hMargin = 15);
        ctxTriples(ctx, statsLeague.allAttackTypes, 'total', posH.triple, posAll, myFont, config.canvasFontSize.smallMedium, config.canvasFontSize.xSmall, color = config.rgb.snowWhite, hMargin = 10, vMargin = 0);
      }
      else if (league == 'mix') { // change TH level
        ctxHitrate(ctx, statsLeague.allAttackTypes, 'total', posH.hitrate[league], posAll, myFont, config.canvasFontSize.smallMedium, config.canvasFontSize.xSmall, hMargin = 30);
        ctxTriples(ctx, statsLeague.allAttackTypes, 'total', posH.hitrate[league], posAll, myFont, config.canvasFontSize.small, config.canvasFontSize.xxSmall, color = config.rgb.gray, hMargin = 20, vMargin = spacing / 4);

        ctxHitrate(ctx, statsLeague.fresh, 'th17', posH.th17, posAll, myFont, config.canvasFontSize.small, config.canvasFontSize.xxSmall, hMargin = 60);
        ctxTriples(ctx, statsLeague.fresh, 'th17', posH.th17, posAll, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxSmall, color = config.rgb.gray, hMargin = 50, vMargin = spacing / 4);

        ctxHitrate(ctx, statsLeague.fresh, 'th16', posH.th16, posAll, myFont, config.canvasFontSize.small, config.canvasFontSize.xxSmall, hMargin = 60);
        ctxTriples(ctx, statsLeague.fresh, 'th16', posH.th16, posAll, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxSmall, color = config.rgb.gray, hMargin = 50, vMargin = spacing / 4);

        ctxHitrate(ctx, statsLeague.fresh, 'th15', posH.th15, posAll, myFont, config.canvasFontSize.small, config.canvasFontSize.xxSmall, hMargin = 60);
        ctxTriples(ctx, statsLeague.fresh, 'th15', posH.th15, posAll, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxSmall, color = config.rgb.gray, hMargin = 50, vMargin = spacing / 4);

        ctxHitrate(ctx, statsLeague.fresh, 'th14', posH.th14, posAll, myFont, config.canvasFontSize.small, config.canvasFontSize.xxSmall, hMargin = 60);
        ctxTriples(ctx, statsLeague.fresh, 'th14', posH.th14, posAll, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxSmall, color = config.rgb.gray, hMargin = 50, vMargin = spacing / 4);

        ctxHitrate(ctx, statsLeague.fresh, 'th13', posH.th13, posAll, myFont, config.canvasFontSize.small, config.canvasFontSize.xxSmall, hMargin = 60);
        ctxTriples(ctx, statsLeague.fresh, 'th13', posH.th13, posAll, myFont, config.canvasFontSize.xSmall, config.canvasFontSize.xxxSmall, color = config.rgb.gray, hMargin = 50, vMargin = spacing / 4);
      };
    };
  };
  ...
  const attachment = new AttachmentBuilder(pngData, { name: `standings_${league}.png` });
...
async function standingsLandscape(league, standings, leagueStats, strRound) {
  // ***** background image ***** //
  bgUrl = `./image/bg/bg_landscape_${league}.png`;
  const bgImage = await Canvas.loadImage(bgUrl);
  ...
  const lengthLogoJwc = 150;
  const imgJwcLogo = await Canvas.loadImage('./image/JWC.png');
  ctx.drawImage(imgJwcLogo, widthCenter - lengthLogoJwc / 2, heightCanvas - 300, lengthLogoJwc, lengthLogoJwc);
  ...
  const timeUtc = Date.now();
  const timeJst = utcToZonedTime(timeUtc, 'Asia/Tokyo');
  ctx.font = `italic ${config.canvasFontSize.small}px ${myFont}`;
  text = format(timeJst, 'pp, PPPP') + ' [JST]';
  ctx.fillText(text, pos.h211, heightCanvas - 180);
  ctx.font = `italic ${config.canvasFontSize.xSmall}px ${myFont}`;
  text = 'Automatically generated by JWC bot';
  ...
  const lengthStar = 50;
  const imgStar = await Canvas.loadImage(`./image/star2.png`);
  ctx = reflectImage2Canvas(ctx, imgStar, lengthStar, lengthStar, posH.sd.left - lengthStar / 2 - 30, headerHeight - lengthStar / 2);
  ctx = reflectImage2Canvas(ctx, imgStar, lengthStar, lengthStar, posH.sd.right - lengthStar / 2 - 30, headerHeight - lengthStar / 2);
  ...
  await Promise.all(standings.map(async (team, index) => {
    ...
    if (league == 'five') {
      if (strRound == 'QUALIFIER') {
        ...
        if (index < numTeamLeft) {
          ...
          posSide[index] = 'right';
          pos0[index] = marginTop + (index - numTeamLeft) * spacing[posSide[index]];
        };
        ratioSpacing = 0.4;
        ctx.globalAlpha = 0.6;
      }
    ...
    if (posSide[index] == 'left') {
      ctx.fillStyle = config.rgb.black;
      ctx.beginPath();
      ctx.moveTo(pos.h371, pos0[index] - spacing[posSide[index]] * ratioSpacing);
      ctx.lineTo(pos.h21, pos0[index] - spacing[posSide[index]] * ratioSpacing);
      ctx.lineTo(pos.h31, pos0[index] + spacing[posSide[index]] * ratioSpacing);
      ctx.lineTo(pos.h381, pos0[index] + spacing[posSide[index]] * ratioSpacing);
    }
    else if (posSide[index] == 'right') {
      ctx.fillStyle = config.rgb.black;
      ctx.beginPath();
      ctx.moveTo(pos.h32, pos0[index] - spacing[posSide[index]] * ratioSpacing);
      ctx.lineTo(pos.h382, pos0[index] - spacing[posSide[index]] * ratioSpacing);
      ctx.lineTo(pos.h372, pos0[index] + spacing[posSide[index]] * ratioSpacing);
      ctx.lineTo(pos.h22, pos0[index] + spacing[posSide[index]] * ratioSpacing);
    };
    ctx.fill();
    ctx.globalAlpha = 1.0;
    ...
    ctx.fillText(text, posH.teamName[posSide[index]], pos0[index] + spacing[posSide[index]] * (ratioSpacing - 0.1));
    ...
    ctxHitrate(ctx, score.clan.allAttackTypes, 'total', posH.hitrate[posSide[index]], pos0[index], myFont, config.canvasFontSize.smallMedium, config.canvasFontSize.xSmall, hMargin = 15);
    ctxTriples(ctx, score.clan.allAttackTypes, 'total', posH.triple[posSide[index]], pos0[index], myFont, config.canvasFontSize.smallMedium, config.canvasFontSize.xSmall, color = config.rgb.snowWhite, hMargin = 10, vMargin = 0);
  ...
  if (strRound == 'QUALIFIER') {
    ...
    ctxHitrate(ctx, statsLeague.allAttackTypes, 'total', posH.hitrate.right, posAll, myFont, config.canvasFontSize.smallMedium, config.canvasFontSize.xSmall, hMargin = 15);
    ctxTriples(ctx, statsLeague.allAttackTypes, 'total', posH.triple.right, posAll, myFont, config.canvasFontSize.smallMedium, config.canvasFontSize.xSmall, color = config.rgb.snowWhite, hMargin = 10, vMargin = 0);
  };
...
exports.standingsLandscape = standingsLandscape;

function ctxHitrate(ctx, score, th, pos, pos0, myFont, fontSize, fontSizePercent, hMargin) {
  ctx.fillStyle = config.rgb.snowWhite;
  ...
  if (score) {
    ...
    ctx.textAlign = 'left';
    text = `%`;
    ctx.font = `${fontSizePercent}px ${myFont}`;
...
function ctxTriples(ctx, score, th, pos, pos0, myFont, fontSize, fontSize2, color, hMargin, vMargin) {
  ctx.fillStyle = color;

  if (score) {
    ctx.textAlign = 'right';
    text = `${score.nTriple[th]}`;
    ctx.font = `${fontSize}px ${myFont}`;
    ctx.fillText(text, pos + hMargin, pos0 + vMargin);

    ctx.textAlign = 'left';
    text = `/${score.nAt[th]}`;
    ctx.font = `${fontSize2}px ${myFont}`;
...
async function legendStatsR1(client, mongoAcc, iDay) {
  ...
  if (iDay == 'current') {
    ...
    if (daysLength == 1) {
      if (dayStatsNewest.day == seasonData.daysNow) { // 最新のデータが当日
        return { attachment: null, isPerfect: false };
      }
      else { // 最新のデータが前日
        dayStats = dayStatsNewest;
  ...
  ctx.font = `${fontSize.xxSmall}px ${myFontSC}`;
  ...
  ctx.drawImage(imgLogoLegend, widthCenter - lengthLogoLegend / 2, 150, lengthLogoLegend, lengthLogoLegend);

  text = 'Start';
  ctx.font = `${fontSize.xxSmall}px ${myFont}`;
  ctx.fillText(text, widthCenter, 500);

  let tophiesStart = 5000;
  if ((!dayStats || dayStats.day != 1)) {
    if (iDay == 'current') {
      tophiesStart = mongoAcc.legend.current ? mongoAcc.legend.current.trophies : 'no data';
    } else if (iDay == 'previous') {
      tophiesStart = mongoAcc.legend.previousDay == null ? '?' : mongoAcc.legend.previousDay.trophies;
    }
  }
  else {
    tophiesStart = 5000;
  ...
  ctx.font = `${fontSize.xSmall}px ${myFontSC}`;
  ...
  ctx.font = `bold ${fontSize.small}px ${myFontSC}`;
  ctx.fillText(text, widthCenter, 1080);

  ctx.font = `bold ${fontSize.xSmall}px ${myFontSC}`;
  text = 'Attacks';
  ...
  if (dayStats) {
    const dayEvents = (mongoAcc.legend?.events ?? []).filter(log => log.season === dayStats.season && log.day === dayStats.day);
    ...
    // 攻撃ログを処理
    await Promise.all(attackLogs.map(async (log, index) => {
      const posAttacksV = 400 + index * 100;
      await drawImageStars(ctx, functions.countStarsLegend(log.diffTrophies), h91, posAttacksV);
      ctx.fillText(`+${log.diffTrophies}`, h51, posAttacksV);
      sumDiffTrophies.attacks += log.diffTrophies;
    }));

    // 防衛ログを処理
    await Promise.all(defenseLogs.map(async (log, index) => {
      const posDefensesV = 400 + index * 100;
      await drawImageStars(ctx, functions.countStarsLegend(log.diffTrophies), h92, posDefensesV);
    ...
    if (sumDiffTrophies.attacks === 320) {
      ctx.font = `bold ${fontSize.smallMedium}px ${myFontSC}`;
  ...
  // ***** TOTAL ***** //
  ctx.font = `bold ${fontSize.medium}px ${myFontSC}`;
  let totalDiffTrophies = sumDiffTrophies.attacks + sumDiffTrophies.defenses;
  text = totalDiffTrophies < 0 ? String(totalDiffTrophies) : '+' + totalDiffTrophies;
  ...
  ctx.font = `${fontSize.xxxSmall}px ${myFont}`;
  ...
  if (dayStats) {
    ...
    text = `${dayStats.season} Season`;
    ctx.font = `${fontSize.xxSmall}px ${myFontSC}, ${myFontJP}, sans-serif`;
  ...
  // ***** 右下 ***** //
  text = mongoAcc.name;
  ctx.font = `bold ${fontSize.xSmall}px ${myFontSC}, ${myFontJP}, sans-serif`;
  ...
  ctx.font = `bold ${fontSize.xxSmall}px ${myFont}, ${myFontJP}, sans-serif`;
  ctx.fillText(textPilot, h72, heightCanvas - 100);

  // ***** 後処理 ***** //
  const pngData = await myCanvas.encode('png');

  const attachment = new AttachmentBuilder(pngData, { name: 'warProgress.png' });
  ...
  ctx.font = `${fontSize.small}px ${myFontSC}`;
  ctx.fillText(text, widthCenter, 780);
  */
}
exports.legendStatsR1 = legendStatsR1;
...
async function legendHistory(mongoAcc) {
  ...
  ctx.font = `${config.canvasFontSize.xxSmall}px ${myFontSC}`;
  ctx.fillStyle = config.rgb.snowWhite;
  ctx.shadowColor = config.rgb.discordBlack;
  ctx.shadowBlur = 6;
  ctx.shadowOffsetY = 2;
  ctx.lineWidth = 1;
  ...
  const rateAttack = sumAttacks10 > 0 ? Math.round((sumTriples10 / sumAttacks10) * 1000) / 10 : 0;
  const rateDefense = sumDefenses10 > 0 ? Math.round((sumDefTriples10 / sumDefenses10) * 1000) / 10 : 0;

  // ※ サマリーの描画は四つのグラフ配置後、最下段に行う（座標は後段で計算）

  // ***** 左下 ***** //
  text = mongoAcc.name;
  ctx.font = `bold ${config.canvasFontSize.xSmall}px ${myFontSC}, ${myFontJP}, sans-serif`;
  ctx.fillText(text, h71, heightCanvas - 170);
  if (mongoAcc.pilotDC) {
    text = mongoAcc.pilotDC.globalName ?? mongoAcc.pilotDC.username;
    ctx.font = `bold ${config.canvasFontSize.xxSmall}px ${myFont}, ${myFontJP}, sans-serif`;
    ctx.fillText(text, h71, heightCanvas - 100);
  };

  // ***** 右下 ***** //
  if (mongoAcc.legend.current) {
    text = `${mongoAcc.legend.current.id} Season`;
  }
  else {
    text = `Not in Legend League`;
  };
  ctx.font = `${config.canvasFontSize.xxSmall}px ${myFontSC}, ${myFontJP}, sans-serif`;
  ctx.fillText(text, h72, heightCanvas - 170);
  ...
  // 先頭40件（過去→現在順の最初の40日）を取得
  let legendDaysLast40 = legendDaysSorted.slice(0, 40);
  ...
  const chartOptionsLine = {
    scales: {
      x: {
        ...
        title: { display: true, text: 'Days ago', font: fontChartString, color: config.rgb.silverWhite },
      },
      y: {
        position: 'left',
        beginAtZero: false,
        ...
        ticks: { font: fontChartNumber, color: config.rgb.silverWhite, stepSize: 100 },
        border: { color: config.rgb.silverWhite },
        title: { display: true, text: 'Trophies', font: fontChartString, color: config.rgb.silverWhite },
        grid: {
    ...
    plugins: {
      legend: { display: false, labels: { usePointStyle: true, font: fontChartString, color: config.rgb.silverWhite, padding: 15 } },
    },
  };
  ...
  const chartDataStack = {
    ...
    datasets: [
      ...
      },
      {
        ...
        data: cdpDefTrophies,
        type: 'bar',
        backgroundColor: config.rgb.grayTransparent,
        yAxisID: 'y',
        stack: 'trophiesStack',
        borderSkipped: false,
        order: 2,
  ...
  const chartOptionsStack = {
    scales: {
      ...
      y: {
        ...
        min: -320,
        max: 320,
        ticks: { font: fontChartNumber, color: config.rgb.silverWhite, stepSize: 80 },
        border: { color: config.rgb.silverWhite },
        title: { display: true, text: 'Trophies Gained / Lost', font: fontChartString, color: config.rgb.silverWhite },
        stacked: true,
        grid: {
          color: (ctx) => (ctx.tick && ctx.tick.value === 0 ? 'rgba(255,255,255,0.4)' : 'rgba(255,255,255,0.2)'),
  ...
  const chartOptionsBars = {
    scales: {
      ...
      y: {
        ...
        beginAtZero: true,
        max: 8,
        ticks: { font: fontChartNumber, color: config.rgb.silverWhite, stepSize: 4 },
        border: { color: config.rgb.silverWhite },
        title: { display: true, text: 'Triples / Def Triples', font: fontChartString, color: config.rgb.silverWhite },
        grid: {
          color: 'rgba(255,255,255,0.2)',
          lineWidth: 0.5,
        },
  ...
  const chartOptionsBarsAD = {
    scales: {
      ...
      y: {
        ...
        ticks: { font: fontChartNumber, color: config.rgb.silverWhite, stepSize: 4 },
        border: { color: config.rgb.silverWhite },
        title: { display: true, text: 'Attacks / Defenses', font: fontChartString, color: config.rgb.silverWhite },
        grid: {
          color: 'rgba(255,255,255,0.2)',
          lineWidth: 0.5,
        },
  ...
  const chartBars = new ChartJSNodeCanvas({ width: widthChart - 15, height: heightBars });
  const chartLine = new ChartJSNodeCanvas({ width: widthChart, height: heightLine });
  const chartStack = new ChartJSNodeCanvas({ width: widthChart - 35, height: heightStack });
  const chartBarsAD = new ChartJSNodeCanvas({ width: widthChart - 35, height: heightBarsAD });

  const configurationBars = { type: 'bar', data: chartDataBars, options: chartOptionsBars };
  const configurationLine = { type: 'line', data: chartDataLine, options: chartOptionsLine };
  const configurationStack = { type: 'bar', data: chartDataStack, options: chartOptionsStack };
  const configurationBarsAD = { type: 'bar', data: chartDataBarsAD, options: chartOptionsBarsAD };
  ...
  ctx.drawImage(imgChartLine, marginLeftChart + 10, yTopLine, widthChart - 15, heightLine);
  ctx.drawImage(imgChartStack, marginLeftChart, yTopStack, widthChart, heightStack);
  ctx.drawImage(imgChartBars, marginLeftChart + 35, yTopBars, widthChart - 35, heightBars);
  ctx.drawImage(imgChartBarsAD, marginLeftChart + 35, yTopBarsAD, widthChart - 35, heightBarsAD);
  ...
  ctx.fillRect(marginLeftChart + 200, summaryTop, widthChart - 350, summaryHeight);
  ctx.restore();

  const headerFont = `${config.canvasFontSize.xSmall}px ${myFontSC}`;
  const labelFont = `${config.canvasFontSize.xxSmall}px ${myFontSC}`;
  const valueFont = `bold ${config.canvasFontSize.xSmall}px ${myFontSC}`;
  const valueFont2 = `${config.canvasFontSize.xxSmall}px ${myFontSC}`;
  ...
  const attachment = new AttachmentBuilder(pngData, { name: 'legendHistory.png' });

  return attachment;
}
exports.legendHistory = legendHistory;

async function warProgress(mongoWar) {
  let chartData = {};
  let chartOptions = {};

  league = mongoWar.league;
  ...
  if (mongoWar.result != '') {
    if (mongoWar.result.arrAttacksPlus) {
      mongoWar.result.arrAttacksPlus.map((attack, index) => {
        if (attack.attackType != 'remaining') {
          if (attack.action == 'attack') {
            let newStars = attack.arrStarsFlag.filter(element => element == 2).length;
            cdpStars[0].push(Number(cdpStars[0][cdpStars[0].length - 1]) + newStars);
            cdpDestruction[0].push(Number(cdpDestruction[0][cdpDestruction[0].length - 1]) + attack.destruction);
            cdpAverageDestruction.clan.push(Number(cdpDestruction[0][cdpDestruction[0].length - 1]) / (cdpDestruction[0].length - 1));
            let triples = Number(cdpTriples[0][cdpTriples[0].length - 1]);
            if (attack.stars == 3) triples += 1;
            else if (attack.stars == 1) oneStar[0] += 1;
            else if (attack.stars == 0) zeroStar[0] += 1;
            cdpTriples[0].push(triples);
            cdpHitrate.clan.push(Number(cdpTriples[0][cdpTriples[0].length - 1]) / (cdpTriples[0].length - 1) * 100);
          }
          else if (attack.action == 'defense') {
            let newStars = attack.arrStarsFlag.filter(element => element == 2).length;
            cdpStars[1].push(Number(cdpStars[1][cdpStars[1].length - 1]) + newStars);
            cdpDestruction[1].push(Number(cdpDestruction[1][cdpDestruction[1].length - 1]) + attack.destruction);
            cdpAverageDestruction.opponent.push(Number(cdpDestruction[1][cdpDestruction[1].length - 1]) / (cdpDestruction[1].length - 1));
            let triples = Number(cdpTriples[1][cdpTriples[1].length - 1]);
            if (attack.stars == 3) triples += 1;
            else if (attack.stars == 1) oneStar[1] += 1;
            else if (attack.stars == 0) zeroStar[1] += 1;
            cdpTriples[1].push(triples);
            cdpHitrate.opponent.push(Number(cdpTriples[1][cdpTriples[1].length - 1]) / (cdpTriples[1].length - 1) * 100);
          };
        };
      });
    };
  ...
  //** タウンホール画像
  let imgTownHall = { th17: null, th16: null, th15: null, th14: null, th13: null };
  ...
  if (league == 'mix') {
    if (mongoWar.season == 17) {
      let lengthTownHall = { th17: 120, th16: 110, th15: 100, th14: 90, th13: 70 };
      ctx.drawImage(imgTownHall.th13, widthCenter - 175, pos.v181 + 45, lengthTownHall.th13, lengthTownHall.th13);
      ctx.drawImage(imgTownHall.th14, widthCenter - 130, pos.v181 + 32, lengthTownHall.th14, lengthTownHall.th14);
      ctx.drawImage(imgTownHall.th15, widthCenter - 70, pos.v181 + 30, lengthTownHall.th15, lengthTownHall.th15);
      ctx.drawImage(imgTownHall.th16, widthCenter + 0, pos.v181 + 10, lengthTownHall.th16, lengthTownHall.th16);
      ctx.drawImage(imgTownHall.th17, widthCenter + 70, pos.v181 + 5, lengthTownHall.th17, lengthTownHall.th17);
    }
      ...
      ctx.drawImage(imgTownHall[2], widthCenter - lengthTownHall[0] / 2 * 5 + 20, pos.v181 + 40, lengthTownHall[2], lengthTownHall[2]);
      ctx.drawImage(imgTownHall[1], widthCenter - lengthTownHall[0] / 2 * 3 + 10, pos.v181 + 20, lengthTownHall[1], lengthTownHall[1]);
      ctx.drawImage(imgTownHall[0], widthCenter - lengthTownHall[0] / 2 * 1 + 0, pos.v181 + 0, lengthTownHall[0], lengthTownHall[0]);
      ctx.drawImage(imgTownHall[1], widthCenter + lengthTownHall[0] / 2 * 1 + 10, pos.v181 + 20, lengthTownHall[1], lengthTownHall[1]);
      ctx.drawImage(imgTownHall[2], widthCenter + lengthTownHall[0] / 2 * 3 + 20, pos.v181 + 40, lengthTownHall[2], lengthTownHall[2]);
    };
  }
  else {
    let lengthTownHall = 120;
    const imgTownHall = await Canvas.loadImage(`./image/th${config.lvTH}_cam3.png`);
    ctx.drawImage(imgTownHall, widthCenter - lengthTownHall / 2, pos.v181, lengthTownHall, lengthTownHall);
  };

  ctx.font = `${config.canvasFontSize.xSmall}px ${myFontNumber}`;
  if (mongoWar.result != '') {
  ...
  if (resultClan && resultOpponent) {
    ctx.font = `bold ${config.canvasFontSize.xxxxLarge}px ${myFontNumber}`;
    ...
    if (mongoWar.clan_war.clan.penalty) {
      ...
      ctx.font = `bold ${config.canvasFontSize.medium}px ${myFontNumber}`;
    ...
    ctx.textAlign = 'right';
    ctx.font = `${config.canvasFontSize.medium}px ${myFontNumber}`;
    ...
    ctx.textAlign = 'left';
    ctx.font = `${config.canvasFontSize.small}px ${myFontNumber}`;
    text = '%';
  ...
  // ***** STATS ***** //
  heightStats = [pos.v101, pos.v91, pos.v81, pos.v71, pos.v61];
  ...
  if (resultClan && resultOpponent) {
    ...
    if (config.nHit[league] == 2) {
      ...
      averageDestruction[0] = Math.round(resultClan.destruction * mongoWar.result.size / resultClan.allAttackTypes.nAt.total * 10) / 10;
    ...
    if (config.nHit[league] == 2) {
      ...
      averageDestruction[1] = Math.round(resultOpponent.destruction * mongoWar.result.size / resultOpponent.allAttackTypes.nAt.total * 10) / 10;
    ...
    ctx.drawImage(barBlack, pos.h141 - 10, heightStats[0] + marginBarBottom, lBarMax, hBar);
    ctx.drawImage(barBlack, widthCenter + 10, heightStats[0] + marginBarBottom, lBarMax, hBar);
    ctx.drawImage(barBlack, pos.h141 - 10, heightStats[1] + marginBarBottom, lBarMax, hBar);
    ctx.drawImage(barBlack, widthCenter + 10, heightStats[1] + marginBarBottom, lBarMax, hBar);
    ctx.drawImage(barBlack, pos.h141 - 10, heightStats[2] + marginBarBottom, lBarMax, hBar);
    ctx.drawImage(barBlack, widthCenter + 10, heightStats[2] + marginBarBottom, lBarMax, hBar);
    ctx.drawImage(barBlack, pos.h141 - 10, heightStats[3] + marginBarBottom, lBarMax, hBar);
    ctx.drawImage(barBlack, widthCenter + 10, heightStats[3] + marginBarBottom, lBarMax, hBar);
    ctx.drawImage(barBlack, pos.h141 - 10, heightStats[4] + marginBarBottom, lBarMax, hBar);
    ctx.drawImage(barBlack, widthCenter + 10, heightStats[4] + marginBarBottom, lBarMax, hBar);
    ...
    valueLeft = resultClan.allAttackTypes.hitrate.total * 0.01;
    valueRight = resultOpponent.allAttackTypes.hitrate.total * 0.01;
    ctx = drawBar(ctx, barGreenLight, barGreenDark, lBarMax, valueLeft, valueRight, widthCenter, pos.h141, heightStats[0] + marginBarBottom, hBar);

    if (config.nHit[league] == 2) {
      valueLeft = resultClan.fresh.hitrate.total * 0.01;
      valueRight = resultOpponent.fresh.hitrate.total * 0.01;
      ctx = drawBar(ctx, barGreenLight, barGreenDark, lBarMax, valueLeft, valueRight, widthCenter, pos.h141, heightStats[1] + marginBarBottom, hBar);

      valueLeft = resultClan.cleanup.hitrate.total * 0.01;
      valueRight = resultOpponent.cleanup.hitrate.total * 0.01;
      ctx = drawBar(ctx, barGreenLight, barGreenDark, lBarMax, valueLeft, valueRight, widthCenter, pos.h141, heightStats[2] + marginBarBottom, hBar);

      valueLeft = resultClan.overkill.hitrate.total * 0.01;
      valueRight = resultOpponent.overkill.hitrate.total * 0.01;
      ctx = drawBar(ctx, barGreenLight, barGreenDark, lBarMax, valueLeft, valueRight, widthCenter, pos.h141, heightStats[3] + marginBarBottom, hBar);
      ...
      ctx = drawBar(ctx, barLight, barDark, lBarMax, valueLeft, valueRight, widthCenter, pos.h141, heightStats[4] + marginBarBottom, hBar);
    }
    else if (config.nHit[league] == 1) {
      ...
      ctx = drawBar(ctx, barRedLight, barRedDark, lBarMax, valueLeft, valueRight, widthCenter, pos.h141, heightStats[2] + marginBarBottom, hBar);

      valueLeft = zeroStar[0] / mongoWar.result.size;
      valueRight = zeroStar[1] / mongoWar.result.size;
      ctx = drawBar(ctx, barRedLight, barRedDark, lBarMax, valueLeft, valueRight, widthCenter, pos.h141, heightStats[3] + marginBarBottom, hBar);
  ...
  if (mongoWar.result != '') {
    ...
    // Home team
    await Promise.all(mongoWar.clan_war.clan.members.map(async (member, index) => {
      ctx.font = `${fontSizeLineup}px ${myFont}, ${myFontJP}, Meiryo, sans-serif`;
      text = member.name;
      ctx.textAlign = 'center';
      ctx.fillText(text, pos.h101, posLineup + spacing * (member.mapPosition - 1));

      // Town Hall Image
      ctx.drawImage(imgTownHall[`th${member.townHallLevel}`], widthCenter - lengthTh / 2, posLineup + spacing * (member.mapPosition - 1) - lengthTh / 2, lengthTh, lengthTh);

      if (member.attacks.length != 0) {
        if (apm == 1) {
          let starFlag = member.attacks[0].arrStarsFlag;
          await drawStars(ctx, starFlag, lengthStar, pos.h21, 'left', posLineup, spacing, member.mapPosition, member.attacks[0], fontSizeLineup, fontSizeLineup2nd, myFont);
        }
        else if (apm == 2) {
          let starFlag = member.attacks[0].arrStarsFlag;
          await drawStars(ctx, starFlag, lengthStar, pos.h51, 'left', posLineup, spacing, member.mapPosition, member.attacks[0], fontSizeLineup, fontSizeLineup2nd, myFont);
          if (member.attacks.length == 2) {
            let starFlag2 = member.attacks[1].arrStarsFlag;
            await drawStars(ctx, starFlag2, lengthStar, pos.h21, 'left', posLineup, spacing, member.mapPosition, member.attacks[1], fontSizeLineup, fontSizeLineup2nd, myFont);
          };
        };
      };
    }));
    // Away team
    await Promise.all(mongoWar.opponent_war.clan.members.map(async (member, index) => {
      ctx.font = `${fontSizeLineup}px ${myFont}, ${myFontJP}, Meiryo, sans-serif`;
      text = member.name;
      ctx.textAlign = 'center';
      ctx.fillText(text, pos.h102, posLineup + spacing * (member.mapPosition - 1));

      if (member.attacks.length != 0) {
        if (apm == 1) {
          let starFlag = member.attacks[0].arrStarsFlag;
          await drawStars(ctx, starFlag, lengthStar, pos.h22, 'right', posLineup, spacing, member.mapPosition, member.attacks[0], fontSizeLineup, fontSizeLineup2nd, myFont);
        }
        else if (apm == 2) {
          let starFlag = member.attacks[0].arrStarsFlag;
          await drawStars(ctx, starFlag, lengthStar, pos.h22, 'right', posLineup, spacing, member.mapPosition, member.attacks[0], fontSizeLineup, fontSizeLineup2nd, myFont);
          if (member.attacks.length == 2) {
            let starFlag2 = member.attacks[1].arrStarsFlag;
            await drawStars(ctx, starFlag2, lengthStar, pos.h52, 'right', posLineup, spacing, member.mapPosition, member.attacks[1], fontSizeLineup, fontSizeLineup2nd, myFont);
          };
        };
      };
    }));
  ...
  ctx.fill();
  ctx.fillStyle = config.rgb.darkOrange;
  ctx.fillRect(pos.h41 - lxRect / 2, posLegend - circleRadius / 2 + 50, lxRect, lyRect);
  ctx.fillStyle = config.rgb.darkBlue;
  ctx.fillRect(pos.h42 - lxRect / 2, posLegend - circleRadius / 2 + 50, lxRect, lyRect);
  ...
  if (mongoWar.result != '') {
    ...
    const imgChart = await Canvas.loadImage(urlChart);
    ctx.drawImage(imgChart, (widthCanvas - widthChart) / 2 + marginLeftChart, marginTopChart, widthChart, heightChart);
  };
  ...
  let dyA = pos.v161;
  try {
    const imgTeamA = await Canvas.loadImage(`./image/teamLogo/${mongoWar.clan_abbr}.png`);
    ctx = reflectImage2Canvas(ctx, imgTeamA, lengthTeamLogo, lengthTeamLogo, dxA, dyA);
  }
  catch (error) {
    const imgJwc = await Canvas.loadImage(`./image/JWC.png`);
    ctx = reflectImage2Canvas(ctx, imgJwc, lengthTeamLogo, lengthTeamLogo, dxA, dyA);
  };
  let dxB = pos.h102 - lengthTeamLogo / 2;
  let dyB = pos.v161;
  try {
    const imgTeamB = await Canvas.loadImage(`./image/teamLogo/${mongoWar.opponent_abbr}.png`);
    ctx = reflectImage2Canvas(ctx, imgTeamB, lengthTeamLogo, lengthTeamLogo, dxB, dyB);
  }
  catch (error) {
    const imgJwc = await Canvas.loadImage(`./image/JWC.png`);
    ctx = reflectImage2Canvas(ctx, imgJwc, lengthTeamLogo, lengthTeamLogo, dxB, dyB);
  };

  // ***** 左下 ***** //
  ctx.font = `bold ${config.canvasFontSize.medium}px ${myFont}`;
  ...
  if (mongoWar.result != '') {
    ...
    const endTimeJst = utcToZonedTime(endTimeUtc, 'Asia/Tokyo');

    ctx.font = `italic ${config.canvasFontSize.xxSmall}px ${myFont}`;
    if (mongoWar.result.state == 'warEnded') {
...
exports.warProgress = warProgress;

function drawBar(ctx, barLight, barDark, lBarMax, valueLeft, valueRight, widthCenter, posH, height, hBar) {
...
async function drawStars(ctx, starFlag, lengthStar, posH, LorR, posLineup, spacing, mapPosition, attack, fontSizeLineup, fontSizeLineup2nd, myFont) {
  let imgStar = [];
  for (let i = 0; i < 3; i++) {
    ...
    ctx.drawImage(imgStar[i], posH + posLR, posLineup + spacing * (mapPosition - 1) - lengthStar / 2, lengthStar, lengthStar);
  };
  ...
  if (attack.destruction != 100) {
    ...
    ctx.textAlign = 'right';
    ctx.font = `${fontSizeLineup}px ${myFont}`;
    ctx.fillText(text, posH + posLR, posLineup + spacing * (mapPosition - 1) + lengthStar * 0.2);
    text = `%`;
    ctx.textAlign = 'left';
    ctx.font = `${fontSizeLineup2nd}px ${myFont}`;
    ctx.fillText(text, posH + posLR, posLineup + spacing * (mapPosition - 1) + lengthStar * 0.22);
  };
  ...
  ctx.textAlign = 'center';
  ctx.font = `${fontSizeLineup2nd}px ${myFont}`;
  ctx.fillText(String(attack.order), posH + posOrder, posLineup + spacing * (mapPosition - 1) - lengthStar * 0.2);
...
/*
async function legendHistoryTop5(clientMongo) {
  ...
  var cursor = clientMongo.db('jwc').collection('accounts').find(query, options).sort(sort).limit(5);
  let accounts = await cursor.toArray();
  await cursor.close();

  // ***** CANVAS ***** //
  const widthCanvas = 1920;
  const heightCanvas = 1920;
  let myCanvas = Canvas.createCanvas(widthCanvas, heightCanvas);
  let ctx = myCanvas.getContext('2d');
  ...
  // ***** background image ***** //
  const bgImage = await Canvas.loadImage('./image/bgDarkPurple.jpg');
  ctx.drawImage(bgImage, 0, 0, widthCanvas, heightCanvas);
  ...
  const lengthLogoLegend = 200;
  const imgLogoLegend = await Canvas.loadImage('./image/legend.png');
  ctx.drawImage(imgLogoLegend, widthCenter - lengthLogoLegend / 2, 150, lengthLogoLegend, lengthLogoLegend);

  text = `TOP 5 Players`;
  ctx.font = `${config.canvasFontSize.xxSmall}px ${myFontSC}`;
  ctx.fillText(text, h71, 250);
  ...
  let chartLabels = [];
  for (let i = 0; i < 20; i++) {
    chartLabels.push(String(20 - i));
  };

  // ***** Get Data for Chart ***** //
  let maxYscale = 6500;
  let arrDataset = [];
  await Promise.all(accounts.map(async (mongoAcc, index) => {
    const nLogTrophiesHistory = mongoAcc.trophiesHistory.length - 1;
    // ***** Chart Data Points ***** //
    let cdpTrophies = [];
    let cdpAttacks = [];

    if (nLogTrophiesHistory <= 20) {
      for (let i = 0; i < 20 - nLogTrophiesHistory; i++) {
        cdpAttacks.push('NaN');
        cdpTrophies.push('NaN');
      };
      mongoAcc.trophiesHistory.forEach((object, index) => {
        if (index != 0) {
          cdpAttacks.push(object.diffAttackWins);
          cdpTrophies.push(object.trophies);
        };
      });
    }
      ...
      mongoAcc.trophiesHistory.forEach((object, index) => {
        if (index > nLogTrophiesHistory - 20) {
          cdpAttacks.push(object.diffAttackWins);
          cdpTrophies.push(object.trophies);
        };
      });
    ...
    if (index == 0) {
      objDataset = {
        label: `Trophies #${index + 1}`,
        data: cdpTrophies,
        fill: true,
        pointStyle: 'rectRot',
        pointRadius: 0,
        pointBorderColor: config.rgb.purple.discordBlack,
        pointBackgroundColor: config.rgb.purple.default,
        borderWidth: 4,
        borderColor: config.rgb.purple.default,
        yAxisID: 'y',
      };
      ...
      text = `${mongoAcc.name}`;
      ctx.font = `${config.canvasFontSize.xxSmall}px ${myFontSC}, ${myFontJP}, sans-serif`;
      ctx.fillText(text, h72, 180);

      // ***** SEASON ***** //
      if (mongoAcc.legend.current) {
        text = `${mongoAcc.legend.current.id} Season`;
        ctx.font = `${config.canvasFontSize.xxSmall}px ${myFontSC}`;
        ctx.fillText(text, h71, heightCanvas - 150);
      };

      // ***** DAY ***** //
      text = `Day ${mongoAcc.trophiesHistory[mongoAcc.trophiesHistory.length - 1].day}`;
      ctx.font = `${config.canvasFontSize.xxSmall}px ${myFontSC}`;
      ctx.fillText(text, h72, heightCanvas - 150);
    }
    else {
      objDataset = {
        label: `Trophies #${index + 1}`,
      ...
      text = `${mongoAcc.name}`;
      if (index == 1) {
        objDataset.borderWidth = 3;
        objDataset.borderColor = config.rgb.purple.nine;
        ctx.font = `${config.canvasFontSize.xxxSmall}px ${myFontSC}, ${myFontJP}, sans-serif`;
        ctx.fillText(text, h72, 220);
      }
      else if (index == 2) {
        objDataset.borderWidth = 3;
        objDataset.borderColor = config.rgb.purple.seven;
        text = `${mongoAcc.name}`;
        ctx.font = `${config.canvasFontSize.xxxSmall}px ${myFontSC}, ${myFontJP}, sans-serif`;
        ctx.fillText(text, h72, 260);
      }
      else if (index == 3) {
        objDataset.borderWidth = 2;
        objDataset.borderColor = config.rgb.purple.six;
        ctx.font = `${config.canvasFontSize.xxxxSmall}px ${myFontSC}, ${myFontJP}, sans-serif`;
        ctx.fillText(text, h72, 295);
      }
      else if (index == 4) {
        objDataset.borderWidth = 2;
        objDataset.borderColor = config.rgb.purple.half;
        ctx.font = `${config.canvasFontSize.xxxxSmall}px ${myFontSC}, ${myFontJP}, sans-serif`;
        ctx.fillText(text, h72, 330);
      };
    };
    arrDataset.push(objDataset);
  }));
  ...
  const width = widthChart;
  const height = heightChart;
  //const backgroundColour = config.rgb.discordBlack;
  ...
  const imgChart = await Canvas.loadImage(urlChart);
  ctx.drawImage(imgChart, marginLeftChart, marginTopChart, widthChart, heightChart);
...
exports.legendHistoryTop5 = legendHistoryTop5;
*/
