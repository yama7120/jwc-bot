...
export default {
  ...
  async execute(interaction, client) {
    ...
    if (subcommand == "league") {
      ...
      let dbValueClan = await client.clientMongo
        .db("jwc")
        .collection("clans")
      ...
      if (wars.length == 0) {
        ...
        await Promise.all(
          wars.map(async (war, index) => {
            ...
            const dbValueClanOpp = await client.clientMongo
              .db("jwc")
              .collection("clans")
            ...
            if (war.deal) {
              arrDescription[index] +=
                `:calendar: ${war.deal.date} :alarm_clock: ${war.deal.time}\n`;
              arrDescription[index] +=
                `:hourglass_flowing_sand: ${war.deal.prep_time} / :crossed_swords: ${war.deal.battle_time}\n`;
            } else {
