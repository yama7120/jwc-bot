...
let data = new SlashCommandBuilder()
  ...
  .addSubcommand(subcommand =>
    subcommand
      ...
      .addStringOption(option =>
        option
          .setName('flag_current')
          .setDescription('現時点を見たい場合は TRUE に（デフォルト：切り替え時）')
          .addChoices(
...
choices_nDisplay.forEach(choice => {
  data.options[3].options[1].addChoices(choice);
  data.options[4].options[1].addChoices(choice);
});
...
async function history(interaction, clientMongo) {
  ...
  let mongoAcc = await clientMongo.db('jwc').collection('accounts').findOne(
    { tag: iPlayerTag },
    { projection: { name: 1, pilotDC: 1, legend: 1, leagueTier: 1, _id: 0 } }
  );
...
async function japanLocal(interaction, client) {
  let flagCurrent = await interaction.options.getString('flag_current');
  let nDisplay = await interaction.options.getInteger('n_display');

  const arrEmbed = await functions.createEmbedLegends(client, flagCurrent, nDisplay, config_coc.locationId.japan);
...
async function global(interaction, client) {
  ...
  const arrEmbed = await functions.createEmbedLegends(client, flagCurrent, nDisplay, 'global');
